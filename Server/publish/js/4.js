      ...
      if(data1.kassas.length>0){
        ...
        for(var i=0; i<klen; i++){
          if(data1.kassas[i].sklad_id==$("#my_sklad").val()){
            ...
            kassa_ip_port=data1.kassas[i].kassa_ip_port;
            break;
          }
        }
        if(numDevice!=0 && PayByProcessing){
          ...
          payData.kassa_ip_port=kassa_ip_port;
...
function select_payment_type_from_zakaz(company_id,zakaz_id,summ=0){	
	api_query("/api/index.php","some_form","get_payment_types").then(function(data){
      ...
      api_query_array("/api/index.php",send_zakaz,"get_zakaz").then(function(zakaz_data){
        ...
        var zakaz_paid=0;
        if(typeof(zakaz_data.zakaz_payments)!="undefined"){
          for(var i=0; i<zakaz_data.zakaz_payments.length; i++){
            zakaz_paid+=parseFloat(zakaz_data.zakaz_payments[i].summ);
          }
        }
        table+='</select></td></tr>';
        table+='<tr><td>Сумма оплаты:</td><td><input type="text" name="select_payment_summ_from_zakaz" id="select_payment_summ_from_zakaz" class="form-control input-sm" value="'+(summ>0?summ:parseFloat(zakaz_data.zakaz.zakaz_sum)-zakaz_paid)+'"></td></tr>';
        ...
        table+='<tr><td>Сумма заказа:</td><td>'+zakaz_data.zakaz.zakaz_sum+'</td></tr>';
        ...
        table+='<tr><td><button type="button" class="btn btn-sm btn-success" onclick="save_payment_from_zakaz('+company_id+','+zakaz_id+',\''+(parseFloat(zakaz_data.zakaz.zakaz_sum)-zakaz_paid)+'\');">Оплатить</button></td>';
      ...
      case 2: //оплата картой через терминал
        api_query("/api/index.php","some_form","get_active_kassas").then(function(data1){
          ...
          if(data1.kassas.length>0){ //&& send['no_fiscalize']==false){
            ...
            var kassa_ip_port="http://localhost:5893";
            for(var i=0; i<klen; i++){
  ...
  function cash_desk_sverka(id,date){
    ...
    api_query_array("/api/index.php",send,"get_cash_desk_sverka").then(function(data){
      if(data.status=="ok"){
        ...
        table+='<tr><td colspan="4">Баланс кассы на начало дня</td><td><b>'+(data.cashdesk_history.yesterday===null?0:data.cashdesk_history.yesterday.summ)+'</b></td></tr>';
        ...
        var klas='';
        if(Math.round(parseFloat((data.cashdesk_history.yesterday===null?0:data.cashdesk_history.yesterday.summ))+razn)!=Math.round(parseFloat(data.cashdesk_history.today.summ))){
          klas="blink_me_red";
        }
        else klas='';
        table+='<tr><td colspan="4">Расчетный баланс кассы на конец дня</td><td style="text-align:right"><b class="'+klas+'">'+(parseFloat((data.cashdesk_history.yesterday===null?0:data.cashdesk_history.yesterday.summ))+razn).toFixed(2)+'</b></td></tr>';
        if(Math.round(parseFloat((data.cashdesk_history.yesterday===null?0:data.cashdesk_history.yesterday.summ))+razn)!=Math.round(parseFloat(data.cashdesk_history.today.summ))){
