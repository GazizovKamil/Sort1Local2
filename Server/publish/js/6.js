...
function get_sklads(){
 api_query("/api/index.php","some_form","get_sklads").then(function(data){
    ...
    for (var i=0; i<datalen; i++){
        ...
        <input type='hidden' name='show_no_photo' value='false'>\
...
function get_sklad_details(sklad_form){
 api_query("/api/index.php",sklad_form,"get_sklad_details").then(function(data){
	...
	table += "<td><form id='delete_sklad_detail_"+data.sklad_details[i].detail_id+"'>\
  <input type=\"hidden\" name=\"detail_id\" value=\""+data.sklad_details[i].detail_id+"\">\
  <input type=\"hidden\" name=\"sklad_id\" value=\""+data.sklad_details[i].sklad_id+"\"></form>";
...
function invent_sklad(sklad_id){
  $.blockUI({ css: { 
    border: 'none', 
    padding: '15px', 
    backgroundColor: '#000', 
    ...
    message: 'Провожу инвентаризацию склада по документам...<a class="pull-right" onclick="$.unblockUI();" style="color:white;">x</a>'
  });
  var send=[];
  send['sklad_id']=sklad_id;
  api_query_array("/api/index.php",send,"invent_sklad").then(function(data){
    if(data.status=="ok"){
      $.unblockUI();
      var i=1;
      var table='<table class="table table-hover" style="width:950px;"><thead><th>№</th><th>артикул</th><th>бренд</th><th>наименование</th><th>кол-во на складе</th><th>должно быть по документам</th></thead><tbody>';
      for(var v of data.sklad_details){
        table+='<tr><td>'+i+'</td><td>'+v.article+'</td><td>'+v.brand+'</td><td>'+v.name+'</td><td>'+v.count+'</td><td>'+v.calculated_count+'</td></tr>';
        i++;

      }
      table+='</tbody></table>';
      create_window("invent_sklad_details_"+sklad_id+"_div","Детали с несоответствием количества","invent_sklad_details_"+sklad_id,table);
      $("#invent_sklad_details_"+sklad_id+"_div").css("left","-100px;")
      //get_sklad_details1(sklad_form);
    }
  });
}
...
function get_sklad_details1(sklad_form){
 ...
 api_query("/api/index.php",sklad_form,"get_sklad_details").then(function(data){
    ...
    <button class='btn btn-sm btn-primary' onclick='invent_sklad("+data.sklad_id+");' title='Произвести инвентаризацию по документам (проверить соответствие количества на складе и движения товара по документам)'>Провести инвентаризацию</button>\
    </center><div id='sklad_doubles_"+data.sklad_id+"'></div><div id='invent_sklad_details_"+data.sklad_id+"'></div></div>";
    ...
    table += "<span class='input-group-addon' id='sklad_search_no_photo_"+data.sklad_id+"'>\
      <input type='checkbox' aria-label='Checkbox for following text input' name='show_no_photo' onchange='$(\"#"+sklad_form+" [name=show_no_photo]\").val($(\"#sklad_search_no_photo_"+data.sklad_id+" [name=show_no_photo]\").prop(\"checked\"));' ";
    if (data.hasOwnProperty('show_no_photo') && data.show_no_photo==1) table += " checked='checked'";
    table+="> Без фото\
    </span>";
    ...
    for (var i=0; i<datalen; i++){
        ...
        <td nowrap>\
            <a onclick='navigator.clipboard.writeText(\""+data.sklad_details[i].article+"\");'><img src='/new_images/copy.png' style='width:18px;' title='скопировать артикул в буфер обмена'></a>\
        ...
        table += "<div class='btn-group' style='display: flex;'>\
            <a href='/print_price_for_detail.php?sklad_detail_id="+data.sklad_details[i].detail_id+"&sklad_id="+data.sklad_id+"' target='_blank' title='Печать ценника детали'><img src='/new_images/printer.svg' class='menuimg'></a>\
            <a href='/print_price_tag_for_detail.php?sklad_detail_id="+data.sklad_details[i].detail_id+"&sklad_id="+data.sklad_id+"' target='_blank' title='Печать ценника на витрину'><img src='/images/price-tag.png' class='menuimg'></a>\
            ...
            <a onclick=\"bootbox.confirm('Вы точно хотите удалить деталь с заказа?',function(result){ if(result) delete_sklad_detail("+data.sklad_details[i].detail_id+","+data.sklad_id+");})\"><img src='/new_images/garbage.svg' class='menuimg'></a>";
...
function edit_price_type_sklad(sklad_id){
  
  var type=$("#price_type option:selected").attr('type');
  var id=$("#price_type option:selected").attr('value');
  if($("#edit_price_type_"+id+"sklad").length==0)
    $("#price_types_sklad").append('<div id="edit_price_type_'+id+'sklad"></div>');
  edit_price_type(id,type,"sklad");
}
...
function edit_sklad(sklad_form){
  api_query("/api/index.php",sklad_form,"get_sklad").done(function(data1){
    ...
    var data_html='\
    <div id="new_price_type_2sklad"></div>\
    <div id="new_price_type_4sklad"></div>\
  ...
  <div class="form-group row">\
              <label for="price_type" class="col-sm-4 col-form-label">Тип цен (наценка склада)</label>\
              <div class="col-sm-8 pull-right">\
                <div id="select_sklad_price_type_type"></div>\
                <div class="input-group" style="width:100%">\
                  ...
                  for(var i=0; i<pt_len; i++){
                      data_html+='<option value="'+data1.price_types[i].id+'" markup="'+data1.price_types[i].proc+'" type="'+data1.price_types[i].type+'"';
                  ...
                  <div class="input-group-btn" style="width: 50px;">\
                    <button title="редактирование наценки" class="btn btn-default" onclick="edit_price_type_sklad('+data.id+');"" type="button">\
                      <img src="/new_images/edit.svg" style="height:17px;">\
                    </button>\
                    <button title="добавление новой наценки" class="btn btn-default" onclick="select_sklad_price_type_type();" type="button">+</button>\
                  </div>\
                </div>\
                <div id="price_types_sklad"></div>\
              </div>\
            </div>\
  <div class="form-group row">\
  ...
  <div class="form-group row">\
	    <label for="search_in_shop" class="col-sm-4 col-form-label">Участвует в поиске в интернет магазине</label>\
	    <div class="col-sm-8">\
		<input type="checkbox" class="search_str" id="search_in_shop" name="search_in_shop"';
	    if(data.search_in_shop==1) data_html+=' checked="checked"';
        data_html+='  onchange="check_sklad_checkboxes();">    </div>\
	</div>\
  <div class="form-group row">\
...
function select_sklad_price_type_type(){
  var table='<table class="table table-hover"><tbody>\
  <tr onclick="add_new_price_type(2,\'sklad\');close_window(\'select_sklad_price_type_type\');"><td>Обычная наценка</td></tr>\
  <tr onclick="add_new_price_type(4,\'sklad\');close_window(\'select_sklad_price_type_type\');"><td>Дифференцированная наценка</td></tr>\
  </tbody></table>';
  create_window("select_sklad_price_type_type_div","Выберите тип наценки","select_sklad_price_type_type",table);
}
...
function edit_sklad_detail(detail_form){
  api_query("/api/index.php",detail_form,"get_sklad_detail").done(function(data1){
    ...
    new_detail_content+="<td><div id='photo_album_"+data.detail_id+"'></div>";
    ...
    new_detail_content+="</td></tr>";
    new_detail_content+="<tr><td>Картинки: </td><td><button type='button' class='btn btn-info btn-sm' onclick='show_photo_album("+data.detail_id+","+data1.sklad_id+", 0);'>Просмотреть</button></td></tr>";
...
function show_photo_album(detail_id, sklad_id, price_id) {
  var send = [];
  send['detail_id'] = detail_id;

  api_query_array("/api/index.php", send, "get_detail_photos").then(function (response) {
    if (response.status !== "ok") {
      alert("Ошибка загрузки фотографий: " + response.msg);
      return;
    }

    const images = response.data;
    let album_html = "<div style='width:450px'><form id='photo_album_form'>";

    album_html += "<div class='mb-3'>";
    album_html += "<button type='button' class='btn btn-success btn-sm' style='margin-bottom: 10px' onclick='add_new_photo(" + detail_id + ","+ sklad_id +","+ price_id +")'>Добавить фото</button>";
    album_html += "<div id='add_photo_modal_" + detail_id + "'></div>";
    album_html += "</div>";

    album_html += `<div style="display: flex; flex-wrap: wrap; gap: 20px;">`;

    images.forEach((image) => {
      const clean_image_url = image.image_url ? image.image_url.split('/')[1] : "";
      const src = clean_image_url ? `https://crossdata.pro/api/get_image?image=${clean_image_url}` : "/no-image.png";
      const isLibrary = image.is_sort1 == 1;
      const isPublic = image.is_public == 1;
      const isPrivate = !isPublic;
      const isActive = image.is_active == 1;

      album_html += `
        <div class="photo-wrapper" data-photo-id="${image.id}" style="
          position: relative;
        ...
        ">
          ...
          <img src="${src}" style="max-width: 100%; max-height: 100%; object-fit: contain; cursor: zoom-in;" onclick="open_fullscreen_image('${src}')">

          <div id="fullscreen_modal" onclick="close_fullscreen_image()" style="
            display: none;
            position: fixed;
            top: 0; left: 0;
            ...
            cursor: zoom-out;
          ">
            <img id="fullscreen_img" style="max-width: 90%; max-height: 90%;" />
          </div>
          ...
          <input type="checkbox" ${isActive ? "checked" : ""} onchange="togglePhotoActive(this)" style="
            position: absolute;
            top: 5px;
            left: 5px;
            z-index: 10;
          " title="Активность изображения">

          ${!isLibrary ? `
            <button type="button" class="btn btn-danger btn-sm"
              style="position: absolute; top: 5px; right: 5px; padding: 0.2em 0.5em; z-index: 10;"
              onclick="delete_details_photo(${image.id}, ${detail_id}, ${sklad_id}, ${price_id})">✕</button>` : ''}
          ...
          ${!isLibrary ? `
            <button type="button"
              ...
              onclick="togglePhotoPublic(this)"
              style="
                position: absolute;
                bottom: 5px;
                right: 5px;
                background: rgba(255,255,255,0.9);
                border: 1px solid ${isPublic ? '#28a745' : '#dc3545'};
                border-radius: 3px;
                padding: 2px 4px;
                font-size: 12px;
    ...
    album_html += `</div></form></div>`;

    create_window("photo_album_div_" + detail_id, "Альбом фотографий", "photo_album_" + detail_id, album_html);
  });
}

function open_fullscreen_image(src) {
  const modal = document.getElementById("fullscreen_modal");
  const img = document.getElementById("fullscreen_img");
  img.src = src;
...
function togglePhotoActive(checkbox) {
  const wrapper = checkbox.closest(".photo-wrapper");
  const overlay = wrapper.querySelector(".photo-overlay");

  const photo_id = wrapper.getAttribute("data-photo-id");
  const is_active = checkbox.checked ? 1 : 0;

  overlay.style.display = is_active ? "none" : "block";
  ...
  api_query_array("/api/index.php", send, "toggle_photo_active").then(function (data) {
    if (data.status !== "ok") {
...
function togglePhotoPublic(button) {
  const wrapper = button.closest(".photo-wrapper");
  const photo_id = wrapper.getAttribute("data-photo-id");
  const is_currently_public = button.getAttribute("data-public") === "1";
  const is_public = is_currently_public ? 0 : 1;
  ...
  api_query_array("/api/index.php", send, "toggle_photo_public").then(function (data) {
    if (data.status === "ok") {
      button.innerText = is_public ? "Public" : "Private";
      button.setAttribute("data-public", is_public);
      button.style.borderColor = is_public ? "#28a745" : "#dc3545";
      button.style.color = is_public ? "green" : "red";
    }
  });
}

function add_new_photo(detail_id, sklad_id, price_id) {
  const modal_content = `
    <div id="upload_photo_container_${detail_id}">
      <input type="hidden" id="photo_detail_id_${detail_id}" value="${detail_id}">
      <div class="form-group">
        <label for="photo_files_${detail_id}">Выберите фото (можно несколько):</label>
        <input type="file" id="photo_files_${detail_id}" class="form-control" accept="image/*" multiple onchange="preview_selected_photos(${detail_id})">
      </div>
      <div class="form-group" style="margin-top: 1em;">
        <label>Тип доступа:</label><br>
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="access_type_${detail_id}" value="private" checked>
          <label class="form-check-label">Приватное</label>
        </div>
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="access_type_${detail_id}" value="public">
          <label class="form-check-label">Публичное</label>
        </div>
      </div>
      <div class="form-group" id="photo_preview_container_${detail_id}" style="margin-top: 1em; display: flex; gap: 10px; flex-wrap: wrap;"></div>
      <div class="form-group text-right" style="margin-top: 1.5em;">
        <button type="button" class="btn btn-primary btn-sm" onclick="submit_photo_upload(${detail_id}, ${sklad_id}, ${price_id})">Загрузить</button>
        <button type="button" class="btn btn-secondary btn-sm" onclick="$('#add_photo_modal_${detail_id}').remove()">Отмена</button>
      </div>
    </div>
  `;

  create_window("add_photo_modal_div_" + detail_id, "Добавить фотографии", "add_photo_modal_" + detail_id, modal_content);
}

function submit_photo_upload(detail_id, sklad_id, price_id) {
  const container = document.getElementById(`upload_photo_container_${detail_id}`);
  if (!container) {
    alert("Контейнер не найден!");
    return;
  }

  const accessTypeInput = container.querySelector(`input[name="access_type_${detail_id}"]:checked`);
  const accessType = accessTypeInput ? accessTypeInput.value : "public";

  const photoPreviewContainer = document.getElementById(`photo_preview_container_${detail_id}`);
  const previewImages = photoPreviewContainer ? photoPreviewContainer.querySelectorAll('img') : [];

  const photoFiles = [];
  previewImages.forEach(image => {
    if (image.dataset.base64) {
      photoFiles.push(image.dataset.base64);
    }
  });
  ...
  api_query_array("/api/index.php", send, "upload_detail_images").then(function (data) {
    if (data.status === 'ok') {
      alert('Фото успешно загружены!');
      $(`#add_photo_modal_${detail_id}`).remove();
      show_photo_album(detail_id, sklad_id, price_id);
    } else {
...
function preview_selected_photos(detail_id) {
  const input = document.getElementById(`photo_files_${detail_id}`);
  const previewContainer = document.getElementById(`photo_preview_container_${detail_id}`);

  if (!input || !previewContainer || !input.files.length) return;
...
function delete_details_photo(photo_id, detail_id, sklad_id, price_id) {
  bootbox.confirm({
    ...
    callback: function (result) {
      if (result) {
        ...
        api_query_array("/api/index.php", send, "delete_detail_photo").then(function (data) {
          if (data.status === 'ok') {
            show_photo_album(detail_id, sklad_id, price_id);
          } else {
            bootbox.alert("Ошибка при удалении: " + data.msg);
          }
        }).catch(function (error) {
          bootbox.alert("Ошибка запроса: " + error);
        });
      }
    }
  });
...
function edit_detail_location(sklad_id,detail_id,topology_id,location,detail_form,location_id,document_details_id,znak){
  ...
  api_query_array("/api/index.php",send,"get_sklad_detail_location").then(function(data){
      ...
      data.topology_levels.forEach(function(item,index){
          ...
          table+='<select name="'+item.name+'" class="form-control">';
      ...
      table+='<tr><td>Кол-во:</td><td><input name="count" value="'+data.sklad_detail_location.count+'"  class="form-control"></td></tr>';
...
function set_sklad_detail_location(sklad_id,detail_id,topology_id,detail_form,document_details_id,znak){
    ...
    api_query_array("/api/index.php",send,"get_sklad_topology").then(function(data){
        ...
        table+='<tr><td>Кол-во:</td><td><input name="count" value="" class="form-control"></td></tr>';
        if(typeof(document_details_id)!="undefined"){
            table+='<tr><td><button type="button" class="btn btn-success" onclick="save_sklad_detail_location(\'document_detail_location\',\''+detail_form+'\',\''+znak+'\');">Сохранить</button></td>';
            table+='<td><button type="button" class="btn btn-default pull-right" onclick="close_window(\'document_detail_topology_select\');">Отмена</button></td></tr>';
...
function edit_price_list_detail(detail_form){
  api_query("/api/index.php",detail_form,"get_price_list_detail").done(function(data1){
    ...
    new_detail_content+="<tr><td><div id='photo_album_"+data.detail_id+"'></div></td></tr>";
    ...
    new_detail_content+="<tr><td>Картинки: </td><td><button type='button' class='btn btn-info btn-sm' onclick='show_photo_album("+data.detail_id+", 0,"+data1.price_list_id+");'>Просмотреть</button></td></tr>";
      ...
      if (data!=null){
        ...
        if(datalen==1){
          change_new_detail_brand2(data.brands[0].brand_id,data.brands[0].detail_id,data.brands[0].brand,typeof(data.brands[0].name)=="string"?data.brands[0].name.replace("'","").replace('"',''):"");
            ...
            for (var i=0; i<datalen; i++){
                  table+="<tr style='cursor:pointer' onclick='change_new_detail_brand2("+data.brands[i].brand_id+","+data.brands[i].detail_id+",\""+data.brands[i].brand+"\",\""+(typeof(data.brands[0].name)=="string"?data.brands[0].name.replace("'","").replace('"','').replace("'",""):"")+"\");'>\
                  <td>"+data.brands[i].article+"</td>\
                  <td>"+data.brands[i].brand+"</td>\
                  <td>"+data.brands[i].name+"</td></tr>";
            }
            table+="</table>";
            if(datalen>1)
...
function get_price_list_details(price_list_form){
    api_query("/api/index.php",price_list_form,"get_price_list_details").then(function(data){
        ...
        table += " onchange='$(\"#"+price_list_form+" [name=search]\").val($(this).val());'></span>";
        
        // Исправленный чекбокс "Без фото"
        table += "<span class='input-group-addon' id='price_list_no_photo_"+data.price_list_id+"'>\
            <input type='checkbox' aria-label='Без фото' name='show_no_photo' onchange='$(\"#"+price_list_form+" [name=show_no_photo]\").val(this.checked ? 1 : 0);' ";
        if (data.hasOwnProperty('show_no_photo') && data.show_no_photo==1) table += " checked='checked'";
        table+="> Без фото\
        </span>";
        ...
        for (var i=1; i<=data.price_list_pages; i++){
            ...
            if (x==1) {
                if (xx==0){
                    ...
                    if($('#price_list_no_photo_'+data.price_list_id+' [name=show_no_photo]').prop('checked')) table += '$(\'#'+price_list_form+' input[name=show_no_photo]\').val(1);';
                    else table += '$(\'#'+price_list_form+' input[name=show_no_photo]\').val(0);';
...
function get_sklad_prices(){
  ...
  function get_sklad_price_details(sklad_price_id,sklad_price_status){
    ...
    send['search_brand']=$("#sklad_price_search_brand").val();
    ...
    api_query_array("/api/index.php",send,"get_sklad_price_details").then(function(data){
      if(data.status=="ok"){
        ...
        <div class="input-group input-group-sm pull-right">\
          ...
          table+='">\
          <label for="sklad_price_search_brand" class="input-group-addon">Бренд:</label>\
          <input class="form-control" type="text" name="search_brand" id="sklad_price_search_brand" onchange="get_sklad_price_details('+sklad_price_id+','+sklad_price_status+');" value="';
        ...
        table+='<div style="height: 77vh; overflow:auto;">\
        ...
        for (var i=0; i<len; i++){
          ...
          <input type="text" size="5" name="sklad_price_det_sale_price_'+data.sklad_price_details[i].id+'" id="sklad_price_det_sale_price_'+data.sklad_price_details[i].id+'"\
        ...
        table+='</tbody></table><div>';
        $.unblockUI();
        create_window_centered_blue("sklad_price_details_"+sklad_price_id+"_div","Печать ценников","sklad_price_details_"+sklad_price_id,table);
      }
    }, function(data){
      $.unblockUI();
    })
  ...
  function get_price_export(id){
    var send=[];
      ...
      api_query_array("/api/index.php",send,"get_price_export").then(function(data){
          ...
          price_exports[data.price_exports[0].id].number_format=data.price_exports[0].number_format;
          price_exports[data.price_exports[0].id].show_field_names=data.price_exports[0].show_field_names;
          price_exports[data.price_exports[0].id].manager_name = data.price_exports[0].manager_name || '';
          price_exports[data.price_exports[0].id].manager_phone = data.price_exports[0].manager_phone || '';
          price_exports[data.price_exports[0].id].photo_on = data.price_exports[0].photo_on;
          if(data.price_exports[0].selected_cols===null || data.price_exports[0].selected_cols==''){
            price_exports[data.price_exports[0].id].selected_cols=[
              ...
              {name:'location',descr:'Местоположение',selected:0}
            ]
          }
          else {
            price_exports[data.price_exports[0].id].selected_cols=JSON.parse(data.price_exports[0].selected_cols);
            if(typeof(price_exports[data.price_exports[0].id].selected_cols[8])=="undefined") {
              price_exports[data.price_exports[0].id].selected_cols[8]={name:'location',descr:'Местоположение',selected:0};
            }
          }
  ...
  function print_price_export_form(export_id){
    ...
    api_query_array("/api/index.php",send,"get_price_types").then(function(data){
      ...
      api_query_array("/api/index.php",send1,"get_price_types").then(function(data1){
        ...
        table+='<tr><td>Поля выгрузки</td><td><input type="checkbox" '+(parseInt(price_exports[export_id].show_field_names)==1?' checked="checked"':'')+' onchange="show_price_export_field_names('+export_id+')"> Выгружать наименования полей</td></tr>';
        table+='<tr><td></td><td>';
        for(var cl in price_exports[export_id].selected_cols){
          table+='<a onclick="order_price_export_field('+export_id+','+cl+',\'up\')">▲</a>';
          table+='<a onclick="order_price_export_field('+export_id+','+cl+',\'down\')">▼</a>';
          table+='<input type="checkbox" '+(price_exports[export_id].selected_cols[cl].selected==1?' checked="checked"':'')+' onchange="select_price_export_field('+export_id+','+cl+')"> '+price_exports[export_id].selected_cols[cl].descr+'<br>';
        }
        table+='</td></tr>';
        table+='<tr><td>формат цены</td><td>\
        <input type="radio" name="number_format" value="1" onchange="set_export_value('+export_id+',\'number_format\',this.value);" '+(price_exports[export_id].number_format==1?" checked":"")+'> 20000.12 (разделитель точка)<br>\
        <input type="radio" name="number_format" value="2" onchange="set_export_value('+export_id+',\'number_format\',this.value);" '+(price_exports[export_id].number_format==2?" checked":"")+'> 20000,12 (разделитель запятая)<br>\
        <input type="radio" name="number_format" value="3" onchange="set_export_value('+export_id+',\'number_format\',this.value);" '+(price_exports[export_id].number_format==3?" checked":"")+'> 20000 (целое число)<br>\
        </td></tr>';
        ...
        api_query("/api/index.php","some_form","get_email_configs").then(function(data2){
          ...
          //if(price_exports[export_id].format=="1"){
            table+='<tr '+(price_exports[export_id].format=="1"?'style="display: table-row"':'style="display: none"')+' id="tr_csv_delimiter"><td>Разделитель полей ( | , - запятая | ; - точка с запятой | по умолчанию - табуляция | )</td><td>\
            ...
            table+='<tr '+(price_exports[export_id].format=="3"?'style="display: table-row"':'style="display: none"')+' id="tr_manager_name"><td>Менеджер (ФИО) *</td><td>\
            <input type="text" class="form-control" id="manager_name" name="manager_name" value="'+price_exports[export_id].manager_name+'" onchange="set_export_value('+export_id+',\'manager_name\',this.value);">\
            </td></tr>';
            table+='<tr '+(price_exports[export_id].format=="3"?'style="display: table-row"':'style="display: none"')+' id="tr_manager_phone"><td>Телефон менеджера *</td><td>\
            <input type="text" class="form-control" id="manager_phone" name="manager_phone" value="'+price_exports[export_id].manager_phone+'" onchange="set_export_value('+export_id+',\'manager_phone\',this.value);">\
            </td></tr>';
            table += '<tr ' + (price_exports[export_id].format == "3" ? 'style="display: table-row"' : 'style="display: none"') + ' id="tr_photo_on">' +
            '<td>Выгружать только с фото</td>' +
            '<td>' +
            '<input type="checkbox" class="form-check-input" id="photo_on" name="photo_on" ' + 
            (price_exports[export_id].photo_on == "1" ? 'checked' : '') + 
            ' onchange="set_export_value(' + export_id + ',\'photo_on\',this.checked ? \'1\' : \'0\');">' +
            '</td></tr>';
          //}
  ...
  function select_price_export_field(export_id,cl){
      price_exports[export_id].selected_cols[cl].selected=!price_exports[export_id].selected_cols[cl].selected;
  }

  function show_price_export_field_names(export_id){
    price_exports[export_id].show_field_names=!parseInt(price_exports[export_id].show_field_names);
  }

  function order_price_export_field(export_id,cl,direction){
    if(((cl+1)>=price_exports[export_id].selected_cols.length && direction=="down") || (cl==0 && direction=="up")){
      return;
    }
    switch(direction){
      case "up": [ price_exports[export_id].selected_cols[cl],price_exports[export_id].selected_cols[(cl-1)] ] = [ price_exports[export_id].selected_cols[(cl-1)],price_exports[export_id].selected_cols[cl] ]; break;
      case "down": [ price_exports[export_id].selected_cols[(cl+1)],price_exports[export_id].selected_cols[cl] ] = [ price_exports[export_id].selected_cols[cl],price_exports[export_id].selected_cols[(cl+1)] ]; break;
    }
    print_price_export_form(export_id);
  }
    ...
    if (typeof(price_exports[export_id][key]) != "undefined") {
      ...
      if (key == "format") {
        if (value == "1") {
          ...
          $("#tr_manager_name").css("display", "none");
          $("#tr_manager_phone").css("display", "none");
          ...
          $("#tr_manager_name").css("display", "table-row");
          $("#tr_manager_phone").css("display", "table-row");
        ...
        if(data.category){
          ...
          if (file_type == 'Авито') {
            ...
            if (file_type == 'yandex') {
              ...
              var link = document.createElement('a');
              link.href = window.URL.createObjectURL(blob);
              link.download = (data.filename!=""?data.filename:"export")+"."+file_type;
              link.click();
            }
          }
        }
  ...
  function get_detail_without_category(price_export_id){
    ...
    api_query_array("/api/index.php", request_params, "get_details_from_without_category").then(function(data){
      if (data.status == "ok") {
        ...
        table += '<span class="input-group-addon" style="cursor: pointer; background: #337ab7; color: white" onclick="price_export_bind_details_to_groups(0, 0, 0, \'stock\',\'' + price_export_id + '\')">Привязать</span>';
        table += '<span class="input-group-addon">';
        table += '<input type="checkbox" onchange="get_detail_without_category('+price_export_id+');" name="show_zero" id="price_export_show_zero_checkbox"';
        if (typeof(data.show_zero) != "undefined" && data.show_zero) table += ' checked';
        table += '>Показать 0 остатки</span>';
        table += '<label for="price_export_search_article_input" class="input-group-addon">Артикул:</label>';
        table += '<input class="form-control" type="text" name="search_article" id="price_export_search_article_input" onchange="get_detail_without_category('+price_export_id+');" value="';
        ...
        table += '<input class="form-control" type="text" name="search_name" id="price_export_search_name_input" onchange="get_detail_without_category('+price_export_id+');" value="';
        ...
        for (var i = 1; i <= data.detail_group_pages; i++) {
          ...
          if (x == 1) {
            if (xx == 0) {
              table += '<li><a href="#" onclick="$(\'#price_export_selected_page_input\').val(\'' + i + '\'); get_detail_without_category('+price_export_id+')">...</a></li>';
            }
            xx++;
          } else {
            if (y == 1) {
              ...
              table += '<li';
              if (selected_page == i) table += " class='active'";
              table += '><a href="#" onclick="$(\'#price_export_selected_page_input\').val(\'' + i + '\'); get_detail_without_category('+price_export_id+')">' + i + '</a></li>';
            }
          }
        }
        table += '</ul></div>';
...
function price_export_bind_details_to_groups(group_id = 0, glubina = 0, force = 0, from = "sklad", price_export_id) {
  if ($("#new_sel_det_group_znak_" + group_id).html() == "-") {
    ...
    if (data.status == "ok") {
      ...
      for (let i = 0; i < len; i++) {
        ...
        var hasInGroup = detailGroup.has_in_group == "1" ? 
          '<a onclick="select_detail_groups_undefined_details_export(' + detailGroup.id + ',' + (glubina) + ',0,\'' + from + '\', \'' + price_export_id + '\');" id="new_sel_det_group_znak_' + detailGroup.id + '">+</a>' 
          : '';

        table += '<tr>\
        <td>' + hasInGroup +
          '<a href="#" onclick="bootbox.confirm(\'Вы точно хотите привязать товары к ' + detailGroup.group_name + '?\', function(result){' +
          'if(result) set_detail_group_undefined_detail_export(' + detailGroup.id + ', \'' + from + '\', \'' + price_export_id + '\');})">' +
...
function set_detail_group_undefined_detail_export(group_id,from="sklad",price_export_id) {
  let send = [];
  ...
  api_query_array("/api/index.php", send, "set_undefined_details_group").then(function (data) {
    selected_details_group = [];
    if (data.status == "ok") {
      get_detail_without_category(price_export_id);
...
function select_detail_groups_undefined_details_export(group_id = 0, glubina = 0, force = 0, from = "sklad", price_export_id) {
  if ($("#new_sel_det_group_znak_" + group_id).html() == "-") {
    $("#new_sel_undefined_detail_groups_list_" + group_id).html('');
    $("#new_sel_det_group_znak_" + group_id).html('+');
    return;
  ...
  api_query_array("/api/index.php", send, "get_detail_groups").then(function (data) {
    if (data.status == "ok") {
      ...
      for (let i = 0; i < len; i++) {
        ...
        table += '<tr>\
          <td>' + hasInGroup + '<a onclick="bootbox.confirm(\'Вы точно хотите привязать товары к ' + detailGroup.group_name + '?\',function(result){ if(result) set_detail_group_undefined_detail_export(' + detailGroup.id + ',\'' + from + '\', \'' + price_export_id + '\');})">' + detailGroup.group_name + '</a></td>\
          <td></td></tr>';
...
function showCategorySelection(category_id, group_id, price_export_id) {
  if ($("#avito_category_znak_" + category_id).html() == "-") {
...
function get_child_category_avito_export(parent_id, glubina, category_id, price_export_id) {
  if ($("#avito_category_znak_" + parent_id).html() == "-") {
  ...
  api_query_array("/api/index.php", send, "get_avito_categorys").then(function (data) {
      if (data.status == "ok") {
          ...
          for (let i = 0; i < len; i++) {
            ...
            // Условие на отображение тега <a>
            if (category.has_children == "1") {
              table += '<a onclick="get_child_category_avito_export(' + category.id + ', ' + glubina + ', ' + category_id + ', ' + price_export_id + ');" id="avito_category_znak_' + category.id + '">+</a> ';
            }
