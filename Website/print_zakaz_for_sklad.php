...
if(isset($_GET['dealer']) && (int)$_GET['dealer']==1){
 ...
 if(isset($_GET['zakaz_id'])){
   ...
   $zakaz_details=$db->getAll("select * from zakaz_details where zakaz_id=?i and reorder_detail_id=0 and (status<100 or status>201) and status<>70 order by zakaz_order",$zakaz_id);
        ...
        <table width="100%" border="1px" celpadding="0" cellspacing="0">
          ...
          foreach($zakaz_details as $zd_key=>$zd_val){
            ...
            echo "  <tr>
              ...
              <td class=\"tc\">".(int)$zd_val['time']." д.</td>
