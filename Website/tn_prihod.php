...
App::run();
 ...
 $sklad_data=$db->getInd("detail_id","select * from sklad_details where sklad_id=?i and detail_id in (?a) and deleted=0",$zakaz_data['sklad_id'],array_column($zakaz_details,"detail_id"));
 $zakaz_details=Search::get_sale_price($zakaz_details,0,'',array(),$db,0);
 ...
 $locations=$db->getInd("detail_id","select detail_id,location from sklad_detail_locations where sklad_id in (select id from sklad where company_id=?i) and detail_id in (?a)",$_SESSION['main_company'],array_column($zakaz_details,"detail_id"));
...
<table border="0" cellpadding="4" cellspacing="0" style="width:29.7cm;">
  ...
  <tr>
    ...
    <td colspan="11" class="td_underline centered">
      <div class="utext" id="tn_gruzopoluchatel">
        <?php
         //echo $client_data['name'].", ИНН ".$client_data['inn'].", КПП ".$client_data['kpp'].", адрес: ".$client_data['address'].", р/с ".$pokupatel_rs_data['rs'].", в банке ".$pokupatel_rs_data['bank'].", БИК ".$pokupatel_rs_data['bik'].", к/с ".$pokupatel_rs_data['ks'];
        ?>
      </div>
  ...
  <tr>
    ...
    <td rowspan="2" class="bordered centered">Местоположение</td>
  ...
  <tr>
    ...
    <td class="bordered centered">склад (факт)</td>
  ...
  <tbody id="tn_content">
    ...
    foreach($zakaz_details as $zd_key=>$zd_val){
      echo '<tr>
        ...
        <td class="bordered centered">'.$zd_val['article'].'</td>
        <td class="bordered centered">шт.</td>
        <td class="bordered centered"></td>
        <td class="bordered centered">'.$locations[$zd_val['detail_id']]['location'].'</td>
        <td class="bordered centered">'.$sklad_data[$zd_val['detail_id']]['count'].'</td>
        <td class="bordered centered"></td>
        <td class="bordered centered"></td>
        <td class="bordered centered">'.$zd_val['count'].'</td>
        <td class="bordered centered" nowrap>';
        if((int)$mainc_taxtype['is_nds']==1){
          echo number_format(round(($zd_val['sale_price'])/(1+$mainc_taxtype['tax_rate']/100),2),2,"."," ");
        }
        else
          echo number_format($zd_val['sale_price'],2,"."," ");
        echo '</td>
        <td class="bordered centered" nowrap>';
        if((int)$mainc_taxtype['is_nds']==1)
          echo number_format(round(($zd_val['sale_price']*$zd_val['count'])/(1+$mainc_taxtype['tax_rate']/100),2),2,"."," ");
        else
          echo number_format(round($zd_val['sale_price']*$zd_val['count'],2),2,"."," ");
        echo '</td>
        ...
        <td class="bordered centered">';
        if((int)$mainc_taxtype['is_nds']==1)
          echo number_format(round(($zd_val['sale_price']*$zd_val['count'])-($zd_val['price']*$zd_val['count'])/(1+$mainc_taxtype['tax_rate']/100),2),2,"."," ");
        else 
          echo "0.00";
        echo '</td>
        <td class="bordered centered">';
        echo number_format($zd_val['sale_price']*$zd_val['count'],2,"."," ");
        echo '</td>
      </tr>';
      $i++;
      $zakaz_sum+=$zd_val['sale_price']*$zd_val['count'];
      $zakaz_count_sum+=$zd_val['count'];
      $sum_without_nds+=round(($zd_val['sale_price']*$zd_val['count'])/(1+$mainc_taxtype['tax_rate']/100),2);
      $sum_nds+=round(($zd_val['sale_price']*$zd_val['count'])-($zd_val['sale_price']*$zd_val['count'])/(1+$mainc_taxtype['tax_rate']/100),2);
    }
    ?>
  </tbody>
