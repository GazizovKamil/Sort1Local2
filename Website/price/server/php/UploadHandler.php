...
class UploadHandler
{
	...
	$file->name = md5($file->name.date("Y-m-d H:i:s"));
        ...
        $file->type = $type;
	$file->price_list_id=stripslashes($this->get_post_param('price_list_id'));
		...
		$db = new SafeMySQL();
		if((int)$file->price_list_id>0){
		    $sql="update price_list set filename='".$file->realname."',localfilename='".$file->name."' where id=?i";
		    $res=$db->query($sql,(int)$file->price_list_id);
		    if ($res) $file->saved=1;
		    else $file->saved=0;
		}
