...
class UploadHandler
{
    ...
    protected function count_file_objects() {
        return count((array)$this->get_file_objects('is_valid_file_object'));
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        $file->name = $this->get_file_name($uploaded_file, $name, $size, $type, $error, $index, $content_range);
	      ...
	      $file->name = md5($file->name.date("Y-m-d H:i:s"));
        $file->size = $this->fix_integer_overflow((int)$size);
        $file->type = $type;
      	if($this->get_post_param('price_list_id')!==null) {$file->base_type="price_list"; $file->base_id=stripslashes($this->get_post_param('price_list_id'));}
      	if($this->get_post_param('sklad_id')!==null) {$file->base_type="sklad"; $file->base_id=stripslashes($this->get_post_param('sklad_id'));}
      	if($this->get_post_param('document_id')!==null) {$file->base_type="document"; $file->base_id=stripslashes($this->get_post_param('document_id'));}
		...
		$db = DB::getInstance();
		if((int)$file->base_id>0){
		    $sql="update ".$file->base_type." set filename='".$file->realname."',localfilename='".$file->name."' where id=?i";
		    $res=$db->query($sql,(int)$file->base_id);
		    if ($res) $file->saved=1;
		    else $file->saved=0;
		}
