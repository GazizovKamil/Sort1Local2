...
class Armtek_brands extends Plugin_brands {
	...
	public function get_curl_handler($force_new = false) {
		...
		if ($is_launch) {
			$url = "https://etp.armtek.ru/search/autocomplete/";
			
			$postfields = "str=".urlencode($this->art)."&type=2&FORMAT=json&LANG=ru"; 
	...
	public function get_cookie($force_new) { 
		$plid = 63;
		...
		if ((int)$info['http_code'] == 200) {
			...
			if ($json->status === true) {
				foreach ($json->data as $ar) {
					$arr[Functions::convert_article($ar->PIN)][] = $ar->BRAND;
				}
			} elseif (($json->status === false) && ($json->arr_messages[0]->type == 'E')&& (preg_match("/нет доступа/ui",$json->arr_messages[0]->text))) {
