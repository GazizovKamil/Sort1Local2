...
session_write_close();
$mfile_data=$db->getRow("select * from proposal_files where id=?i",(int)$file_id);
//echo print_r($mfile_data,true)."\n";
$permition=$db->getOne("select id from proposals where user_id=?i and id=?i",$user_id,(int)$mfile_data['proposal_id']);
